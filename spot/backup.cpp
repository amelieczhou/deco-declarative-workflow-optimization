...
//	errno_t err;
	...
	for(int i=0; i<dags.size(); i++){
		...
		for(; vp.first != vp.second; vp.first++){
			...
			if (i==0)
				probestTime_device[j++]=(*dags[i]->g)[*vp.first].probestTime;//************
		}
	}
	...
	printf ("SIZE: vertex %d edge %d", (*dags[0]->g).m_vertices.size(), (*dags[0]->g).m_edges.size());
	...
	std::cout<<"A* Search CPU started"<<std::endl;
	...
	while(!(Openset.empty() || searchcount > 10000)){
		...
		if(constraint){			 
			if(headnode->fvalue < globalBestCost){
				...
				//Xuntao
				for (int i=0;i<numoftasks;++i)
					printf ("%d ", headnode->configurations[i]);
					printf ("%f\n", headnode->fvalue);
