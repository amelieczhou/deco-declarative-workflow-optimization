    ...
    public Map getSiteMap( String namespace, String name, String version,
        ...
        if ( tcentries != null ) {
            ...
            throw new RuntimeException(
                ...
                lfn +
                "\"in the TC and the tc infos are" + namespace + name + version );
        }
        if ( sysinfomap != null ) {
            for ( Iterator i = siteids.iterator(); i.hasNext(); ) {
                String site = ( String ) i.next();
                SysInfo sitesysinfo = ( SysInfo ) sysinfomap.get( site );
		mLogger.log("current site is "+site+" with sysinfo "+sitesysinfo, LogManager.DEBUG_MESSAGE_LEVEL);
...
//			mLogger.log("txsysinfo is "+txsysinfo, LogManager.DEBUG_MESSAGE_LEVEL);

                    //check for installed and static binary executables at each site.
                    if ( txsysinfo.equals( sitesysinfo ) ) {
			 mLogger.log("txsiteid is "+txsiteid+"txtype is "+txtype, LogManager.DEBUG_MESSAGE_LEVEL);
                        if (
...
//		throw new RuntimeException("sysinfomap is empty for sites "+siteids);
            System.exit( 1 );
        }
